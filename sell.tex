...
\item player \II
chooses 
a premise sequent in which the game continues;
\item  the game splits into independent subgames, where \I has to win all of them if she want to win the game.
\end{enumerate}
\end{itemize}
...
\]
\end{example}
Hence, different proofs of the same sequent can lead to different costs. Nevertheless, it is straightforward to show that cost-optimal strategies exist for all provable sequents. We note, however, that the proof of this result is non-constructive.
...
This result demonstrates that our games and systems provide a more precise control over the resources appearing {\em negatively} in sequents, unlocking new opportunities for analyzing the problem of {\em comparing proofs}. For instance, we anticipate that studying the costs of proofs in labelled calculi could reveal connections between labels and computational bounds~\cite{DBLP:journals/jfp/AccattoliGK20}.
